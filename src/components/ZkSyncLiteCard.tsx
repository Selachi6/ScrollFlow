...
export const ZkSyncLiteCard: FC<{ wallet: WalletInformation | null }> = ({ wallet }) => {
  ...
  return (
    <div>
      <div
        className="relative mt-1.5 rounded-lg border-gray-700 border mb-4 w-[812px] bg-gray-800 text-center"
        style={{ color: 'white' }}
      >
          ...
          <div>
            {wallet ? (
              <div className="p-5 pl-20 pr-20 bg-gray-900 rounded-t-lg flex justify-between">
                <div>
                  <h1>Interactions</h1>
                  <span className="mb-2 text-xl font-extrabold text-blue-600">{wallet!.zksynclite.interactions}</span>
                </div>
                <div>
                  <h1>Last Activity</h1>
                  <span className="mb-2 text-xl font-extrabold text-blue-600">
                    {wallet!.zksynclite.lastActivity ? getTimeAgo(wallet!.zksynclite.lastActivity) : 'Never'}
                  </span>
                </div>
                <div>
                  <h1>Activated On</h1>
                  <span className="mb-2 text-xl font-extrabold text-blue-600">
                    {wallet!.zksynclite.activatedOn ? getTimeAgo(wallet!.zksynclite.activatedOn) : 'Never'}
                  </span>
                </div>
              </div>
            ) : (
              <div className="grid place-items-center mt-2 mb-6">
                <svg
                  aria-hidden="true"
